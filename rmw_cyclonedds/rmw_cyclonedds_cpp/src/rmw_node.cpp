...
#define RMW_VERSION_GTE(major, minor, patch) ( \
    major < RMW_VERSION_MAJOR || (major == RMW_VERSION_MAJOR && (minor < RMW_VERSION_MINOR || (minor == RMW_VERSION_MINOR && patch <= RMW_VERSION_PATCH))))
...
extern "C" const char *rmw_get_implementation_identifier()
{
  ...
  myfile << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count() << ", " << __FUNCTION__ << ", "<< __LINE__ <<"\n";
  myfile.close();
...
static bool check_create_domain_locked(dds_domainid_t did, bool localhost_only)
{
  ...
  else
  {
    ...
    /* Localhost-only: set network interface address (shortened form of config would be
       ...
       comma. */
    std::string config =
        localhost_only ? "<CycloneDDS><Domain><General><NetworkInterfaceAddress>localhost</NetworkInterfaceAddress>"
...
static rmw_ret_t make_names_and_types(
    rmw_names_and_types_t *tptyp, const std::map<std::string, std::set<std::string>> &source,
    rcutils_allocator_t *allocator)
{
  if (source.size() == 0)
  {
